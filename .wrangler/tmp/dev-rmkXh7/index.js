...
// .wrangler/tmp/bundle-vb0HPC/checked-fetch.js
...
var BoxRoom = class {
  ...
  constructor(state) {
    ...
    this.state.blockConcurrencyWhile(async () => {
      const stored = await this.state.storage.get("position");
      if (stored) {
        this.position = stored;
        console.log(`\u{1F4E6} Loaded persisted position: (${this.position.x}, ${this.position.y})`);
      }
    });
  }
  async fetch(request) {
  ...
  handleSession(socket, userId) {
    socket.accept();
    this.sockets.set(socket, userId);
    console.log(`\u{1F535} User connected: ${userId} (Total: ${this.sockets.size})`);
    ...
    socket.addEventListener("message", async (event) => {
      try {
        ...
        if (typeof data.x === "number" && typeof data.y === "number") {
          ...
          const message2 = JSON.stringify(this.position);
          ...
          console.log(`\u{1F4E6} Position updated by ${userId}: (${this.position.x}, ${this.position.y})`);
        }
      } catch (err) {
    ...
    socket.addEventListener("close", () => {
      ...
      console.log(`\u{1F534} User disconnected: ${userId} (Remaining: ${this.sockets.size})`);
    });
...
// src/index.ts
var SUPABASE_URL = "https://klrfbcwwxowsixmijxcc.supabase.co";
...
app.get("/profile", async (c) => {
  ...
  try {
    ...
    console.log(`\u{1F50D} Looking up user: ${userId}`);
    const cachedProfile = await c.env.USER_PROFILES.get(userId, "json");
    if (cachedProfile) {
    ...
    const profile = {
      id: userId,
      email: payload.email || payload.user_metadata?.email || "unknown",
      name: payload.user_metadata?.full_name || payload.user_metadata?.name || "User",
      lastLogin: (/* @__PURE__ */ new Date()).toISOString()
    };
    await c.env.USER_PROFILES.put(userId, JSON.stringify(profile), {
      expirationTtl: 3600
      // 1 hour
    });
    console.log(`\u{1F4BE} Stored profile in KV for user: ${userId}`);
    return c.json({
      profile,
      cached: false
    });
...
app.get("/ws", async (c) => {
  ...
  try {
    ...
    console.log(`\u{1F50C} WebSocket connection attempt for user: ${userId}`);
    ...
    console.log(`\u2705 WebSocket upgraded for user: ${userId}`);
...
// .wrangler/tmp/bundle-vb0HPC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vb0HPC/middleware-loader.entry.ts
